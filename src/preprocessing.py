...
def transform_pdb_to_numpy(pdb_file: str, parser=None, center: bool=False) -> Mapping[str, np.array]:
    """
...
def clean_files(pdb_files_dir='../data/input/refined-set'):
    """
...
def train_test_split(pdb_id_dir='../data/input/refined-set/', train_percentage=0.8):
    ...
    with open('../data/input/train.txt', 'w') as train_ids_file:
        for train_id in train_ids:
            train_ids_file.write(train_id + '\n')

    with open('../data/input/test.txt', 'w') as test_ids_file:
...
def reproduce_preprocessing():
    ...
    train_ids_file = os.path.join(input_dir, 'train.txt')
    test_ids_file = os.path.join(input_dir, 'test.txt')
    ...
    with open(os.path.join(input_dir, 'test.txt'), 'r') as test_id_file:
        for test_id in test_id_file.readlines():
            test_ids.append(test_id.strip('\n'))

    pdb_files_dir = os.path.join(input_dir, 'refined-set')
    ...
    for i, train_id_dir in tqdm(enumerate(train_ids)):
        ...
        for x_file in os.listdir(full_train_id_dir):
            ...
            combined_array = transform_pdb_to_numpy(full_x_file_path, parser)
    ...
    with open('../data/logs/problems_files.txt', 'r') as to_remove:
        for f in to_remove.readlines():
            protein = f.split('/')[-1].split('_')[0]
            full_path_train = os.path.join('../data/input/raw_pdb_arrays/train', protein)
            full_path_test = os.path.join('../data/input/raw_pdb_arrays/test', protein)
